...
- `PREVIEW_MODE=port`（默认，本地联调）：`previewUrl = ${PREVIEW_BASE}/p/<token>/`
  ...
  `--base /p/<token>/` 启动，Vite 对裸 `/p/<token>` 回 404 而非 302，iframe 会拿到空白页；反代另对裸路径补 302 兜底。`PREVIEW_BASE` 必须是含协议与端口的绝对 origin
  （`http://<host>:3100`）；留空会退化成相对路径，iframe 相对父页面解析 → Nuxt 404。
